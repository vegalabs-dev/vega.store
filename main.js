...
let serviciosData=[];let productoSeleccionado={nombre:'',precio:0,meses:1,tipo_ingreso:'correo'};
const numeroWhatsApp="51928293163";const modalCompra=document.getElementById('modal-compra');
const modalTiempo=document.getElementById('modal-tiempo');const inputDatoCompra=document.getElementById('correo-compra');
const alertaDato=document.getElementById('alerta-correo');const otpBoxes=document.querySelectorAll('.otp-box');
...
async function cargarCatalogo(){
    ...
    serviciosData.forEach((serv,index)=>{
        ...
        pillsHTML+=`</div>`;
        contenedor.innerHTML+=`<div class="card">${etiquetaHTML}<h2>${serv.nombre}</h2>${pillsHTML}<p class="price" id="precio-display-${index}">Cargando...</p><ul class="features">${listaCaract}</ul><div class="buttons"><button class="btn-primary" onclick="abrirCompra(${index})">Obtenerlo ahora</button><button class="btn-secondary" onclick="abrirVerTiempo()">Consultar mi servicio</button></div></div>`;
...
window.seleccionarPlan=function(servIndex,planIndex,btnElement){
    ...
    let serv=serviciosData[servIndex];let plan=serv.planes_procesados[planIndex];
    let precioFinal=plan.promo?parseFloat(plan.promo):parseFloat(plan.precio);
    let sufijo=plan.meses==0?'':'<span>/ mes</span>';
    let precioHTML=plan.promo?`<span style="text-decoration:line-through; color:#9CA3AF; font-size:16px;">S/ ${parseFloat(plan.precio).toFixed(2)}</span> S/ ${precioFinal.toFixed(2)} ${sufijo}`:`S/ ${precioFinal.toFixed(2)} ${sufijo}`;
    document.getElementById(`precio-display-${servIndex}`).innerHTML=precioHTML;
    serv.seleccion_actual={nombre:serv.nombre,precio:precioFinal,meses:plan.meses,tipo_ingreso:serv.tipo_ingreso||'correo'};
}

function mostrarNotificacion(mensaje,tipo='error'){
    const contenedor=document.getElementById('toast-container');const toast=document.createElement('div');
...
window.abrirCompra=function(servIndex){
    ...
    if(productoSeleccionado.tipo_ingreso==='numero'){
        inputDatoCompra.placeholder="1. Escribe tu número de WhatsApp";inputDatoCompra.type="tel";alertaDato.innerText="⚠️ Ingresa tu número de WhatsApp primero.";
    }else{
        inputDatoCompra.placeholder="1. Escribe el correo a vincular";inputDatoCompra.type="email";alertaDato.innerText="⚠️ Ingresa tu correo primero.";
    }
    modalCompra.classList.remove('oculto');inputDatoCompra.value="";inputDatoCompra.style.borderColor="#E5E7EB";alertaDato.style.display="none";
    otpBoxes.forEach(box=>box.value='');
}

window.abrirVerTiempo=function(){modalTiempo.classList.remove('oculto');document.getElementById('correo-tiempo').value="";document.getElementById('mensaje-tiempo').innerHTML="";}
...
function validarDatoCompra(){
    const dato=inputDatoCompra.value.trim();let esValido=false;
    if(productoSeleccionado.tipo_ingreso==='numero'){const numLimpio=dato.replace(/\D/g,'');esValido=numLimpio.length>=9;}
...
document.getElementById('btn-confirmar-yape').addEventListener('click',async()=>{
    ...
    const operacion=Array.from(otpBoxes).map(box=>box.value).join('');if(operacion.length<7)return mostrarNotificacion("Ingresa los 7 números de operación.");
    const btn=document.getElementById('btn-confirmar-yape');btn.innerText="Procesando...";btn.disabled=true;
    await supabaseClient.from('usuarios_canva').insert([{correo:datoCliente,servicio:productoSeleccionado.nombre,meses:productoSeleccionado.meses,metodo_pago:'Yape',num_operacion:operacion,estado:'Pendiente'}]);
...
document.getElementById('btn-otro-medio').addEventListener('click',async()=>{
    ...
    await supabaseClient.from('usuarios_canva').insert([{correo:datoCliente,servicio:productoSeleccionado.nombre,meses:productoSeleccionado.meses,metodo_pago:'Otro (Plin/BCP)',token:token,estado:'Pendiente'}]);
    let txtMesesMsg=productoSeleccionado.meses==0?"Pago Único":`${productoSeleccionado.meses} Meses`;
...
document.getElementById('btn-buscar-tiempo').addEventListener('click',async()=>{
    const datoBuscado=document.getElementById('correo-tiempo').value.trim();const msj=document.getElementById('mensaje-tiempo');
    ...
    if(data&&data.length>0){
        ...
        if(usuario.estado==='Activo'){
            if(usuario.meses==0||!usuario.fecha_fin){msj.innerHTML=`✅ Tu cuenta de <b>${usuario.servicio||'Servicio'}</b> está activa.<br><br><strong style="color:var(--primary); font-size:22px;">Acceso Permanente (Pago Único)</strong>`;}
            else{
                let hoy=new Date();let fin=new Date(usuario.fecha_fin);let dias=Math.ceil((fin.getTime()-hoy.getTime())/(1000*3600*24));
